...
| Sell or buy at a price you choose, not the market's | `POST /wallet/v1/limit-orders` - the order rests on 1Click until it fills, you cancel it, or 7 days pass. See "Limit Orders" |
...
On a **multisig** wallet the create call answers `pending_approval` (see "Request Policy"): approvers sign the order's exact terms — the token and amount sent, the token and minimum amount received — and the order is placed only after the threshold.
...
Always send an `Idempotency-Key` (see "Idempotency-Key — one key per operation"): a repeated key is not executed again, so a retry cannot rest — and fund — a second order.
